...
/*
=================================
- Used for updating the boundary
=================================
*/
void cSprite::updateSprite(float deltaTime)
{

	setBoundingRect2(&boundingRect);
...
/*
=================
- Determine the bounding rectangle for the sprite for the Fruits.
=================
*/
...
void cSprite::setBoundingRect(RECT* pRect)
{
	...
	RECT boundingRect;
	boundingRect.left = sPos.x - textureWidth / 2;
	boundingRect.top = sPos.y - textureHeight / 2;
	boundingRect.right = textureWidth / 2 + sPos.x;
	boundingRect.bottom = textureHeight / 2 + sPos.y;

	SetRect(pRect, (int)boundingRect.left + textureWidth / 2, (int)boundingRect.top + textureHeight / 2, (int)boundingRect.right + textureWidth / 2, (int)boundingRect.bottom + textureHeight / 2);
}

/*
=================
- Determine the bounding rectangle for the sprite for the Basket.
=================
*/

void cSprite::setBoundingRect2(RECT* pRect)
{
	glm::vec2 sPos = getSpritePos();
...
RECT cSprite::getBoundingRect2()		// Return the bounding rectangle for the sprite
{
	return cSprite::theBoundingRect;
}

/*
RECT cSprite::getTheBoundingRect()		// Return the bounding rectangle for the sprite
{
	return cSprite::theBoundingRect;
}
*/
...
void cSprite::renderCollisionBox()
{
	...
	glTranslatef(theBoundingRect.left, theBoundingRect.top, 0.0f);
	glRotatef(spriteRotation, 0.0f, 0.0f, 1.0f);
	glScalef(spriteScaling.x, spriteScaling.y, 1.0f);
	...
	//glVertex2f(0, 0);
	//glVertex2f(0, theBoundingRect.bottom);
	//glVertex2f(theBoundingRect.right, theBoundingRect.bottom);
	//glVertex2f(theBoundingRect.right, 0);
	glBegin(GL_QUADS);
	glVertex2f(0, 0);
	glVertex2f(textureWidth, 0);
	glVertex2f(textureWidth, textureHeight);
